...
def rr(processes, time_quantum):
    ...
    return execute_scheduling_algorithm(processes, time_quantum, current_process)
...
def execute_scheduling_algorithm(processes, time_quantum=None,current_process=None):
    current_time = 0
    wait_time_sum = 0
    ...
    while processes:
        ...
        if next_process is None:
            current_time += 1
            continue
        if time_quantum and current_process:
            print(1)
            l = len(processes)
            idx = processes.index(current_process)
            next_process=processes[(idx+1)%l]
